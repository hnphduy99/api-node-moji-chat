...
export const sendFiendRequest = async (req: Request, res: Response) => {
  try {
    //Todo
    ...
    const userExist = User.exists({ _id: to });
...
export const acceptFiendRequest = async (req: Request, res: Response) => {
  try {
    //Todo
    ...
    console.log('Lỗi khi chấp nhận lời mời kết bạn kết bạn', error);
    return res.status(500).json({ message: 'Lỗi hệ thống, vui lòng thử lại sau' });
  }
};

export const declineFiendRequest = async (req: Request, res: Response) => {
  try {
    //Todo
    const { requestId } = req.params;
    const userId = req.user._id;
    ...
    console.log('Lỗi khi từ chối lời mời kết bạn kết bạn', error);
    return res.status(500).json({ message: 'Lỗi hệ thống, vui lòng thử lại sau' });
  }
};
